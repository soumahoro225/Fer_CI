...
const roleLabel = (role: DashboardData["user"]["role"]) => role === "direction" ? "Direction" : role === "agent" ? "Agent technique" : role === "user" ? "Utilisateur" : "Citoyen";
...
export default function DashboardClient({ initialData }: { initialData: DashboardData }) {
  ...
  const currentTitle = active === "Vue d’ensemble" ? initialData.user.role === "user" ? "Mes signalements" : "État du réseau et des financements" : active;
  const visibleNav = initialData.user.role === "user" ? nav.slice(0, 1) : nav;
  ...
  return <main className="app-shell">
    ...
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      ...
      <nav aria-label="Navigation principale"><p>PILOTAGE</p>{visibleNav.map(([label, Icon]) => <button key={label} className={active === label ? "active" : ""} onClick={() => { setActive(label); setMobileOpen(false); }}><Icon size={18} />{label}{label === "Signalements" && <em>{items.length}</em>}</button>)}{initialData.user.role !== "user" && <><p>ADMINISTRATION</p><button disabled title="Bientôt disponible"><Wrench size={18} />Équipes & prestataires</button><button onClick={() => { setAccountOpen(true); setMobileOpen(false); }}><Settings size={18} />Mon compte</button></>}</nav>
    ...
    <section className="workspace">
      ...
      <div className="content"><section className="heading"><div><p>{currentDate}</p><h2>{currentTitle}</h2><span>Les chiffres ci-dessous proviennent de la base GEOSIGNALE-CI.</span></div><div><button ref={newIncidentButton} className="primary" onClick={openIncidentModal}><Plus size={18} />Nouveau signalement</button></div></section>
        ...
        {initialData.user.role !== "user" && <section className="objectives"><span>OBJECTIFS FER</span><div><p><i className={engagementRate <= 100 ? "ok" : "warn"} />Engagements / budget <b>{engagementRate}%</b></p><p><i className={paymentAlerts ? "warn" : "ok"} />Décomptes proches de 60 jours <b>{paymentAlerts}</b></p><p><i className="ok" />Patrimoine inventorié <b>{initialData.assets.length}</b></p><p><i className="ok" />Ressources enregistrées <b>{initialData.resources.length}</b></p></div></section>}
        <section className={`kpis ${initialData.user.role === "user" ? "user-kpis" : ""}`}><article><span className="kicon red"><AlertTriangle /></span><div><small>Mes signalements ouverts</small><strong>{openIncidents}</strong><p>{items.length} signalement(s) au total</p></div></article>{initialData.user.role !== "user" && <><article><span className="kicon amber"><Construction /></span><div><small>Interventions en cours</small><strong>{runningInterventions}</strong><p>{initialData.interventions.length} intervention(s) enregistrée(s)</p></div></article><article><span className="kicon green"><Landmark /></span><div><small>Ressources mobilisées</small><strong>{formatFcfa(totalResources)}</strong><p>Données déclarées dans GEOSIGNALE-CI</p></div></article><article><span className="kicon blue"><WalletCards /></span><div><small>Décomptes à régler</small><strong>{formatFcfa(unpaidAmount)}</strong><p>{unpaid.length} décompte(s) ouvert(s)</p></div></article></>}</section>
    ...
    {modal && <div className="modal-bg" role="presentation" onMouseDown={() => { if (!submitting) setModal(false); }}>
      <form className="modal incident-modal" role="dialog" aria-modal="true" aria-labelledby="incident-title" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
        ...
        {initialData.user.role === "user" ? <input type="hidden" name="severity" value="Modérée" /> : <label>Priorité<select name="severity"><option>Critique</option><option>Élevée</option><option>Modérée</option></select></label>}
