...
import {
  AlertTriangle, BarChart3, Bell, ChevronDown, ClipboardList, Construction, Filter,
  Landmark, Layers3, LogOut, Map, MapPin, Menu, Plus, Search, Settings, ShipWheel,
  Signpost, UserRound, WalletCards, Wrench, X,
} from "lucide-react";
...
export default function DashboardClient({ initialData }: { initialData: DashboardData }) {
  ...
  return <main className="app-shell">
    ...
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      <div className="brand"><div className="brand-mark"><span>FER</span><i /></div><div><strong>FER SIG</strong><small>Pilotage routier</small></div></div>
    ...
    <section className="workspace">
      ...
      <div className="content"><section className="heading"><div><p>{currentDate}</p><h2>{currentTitle}</h2><span>Les chiffres ci-dessous proviennent de la base FER-CI.</span></div><div><button ref={newIncidentButton} className="primary" onClick={() => setModal(true)}><Plus size={18} />Nouveau signalement</button></div></section>
