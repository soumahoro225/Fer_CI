...
const cityFromLocation = (location: string) => location.split(/[,;|]/, 1)[0]?.trim() || "Non précisée";
...
export default function DashboardClient({ initialData }: { initialData: DashboardData }) {
  ...
  const cityOptions = useMemo(
    () => Array.from(new Set(items.map((item) => cityFromLocation(item.location)))).sort((a, b) => a.localeCompare(b, "fr")),
    [items],
  );
  const effectiveCategoryFilter = categoryFilter === "Toutes" || categoryOptions.includes(categoryFilter) ? categoryFilter : "Toutes";
  const effectiveCityFilter = cityFilter === "Toutes" || cityOptions.includes(cityFilter) ? cityFilter : "Toutes";
  const normalizedQuery = query.trim().toLocaleLowerCase("fr");
  const filtered = useMemo(() => items.filter((item) => {
    const matchesQuery = !normalizedQuery || `${item.title} ${item.location} ${item.category} ${item.id} ${reporterContact(item)}`.toLocaleLowerCase("fr").includes(normalizedQuery);
    const matchesCategory = effectiveCategoryFilter === "Toutes" || item.category === effectiveCategoryFilter;
    const matchesCity = effectiveCityFilter === "Toutes" || cityFromLocation(item.location) === effectiveCityFilter;
  ...
  const overviewSelected = selected && filtered.some((item) => item.databaseId === selected.databaseId)
    ? selected
    : filtered[0] ?? null;
  const selectedEvidence = overviewSelected
    ? evidence.filter((item) => item.incident_id === overviewSelected.databaseId)
    : [];
